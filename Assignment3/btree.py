...
class BTree(object):
    ...
    def search(self, value, node=None):
        ...
        if isinstance(node.keys, list):
            for val in node.keys:
                print('Checking if value is equal to ' + str(val))
                if val == value:
                    return True
        if value in node.keys:
            ...
            while i < node.size and value > node.keys[i]:
                # print('Checking if value is equal to ' + str(node.keys[i]))
                i += 1
    ...
    def print_sorted_elements(self):
        """Print the elements of the tree in a sorted manner"""

        node_list = [z.keys for z in self.root.children]
        node_list_1d = [a for element in node_list for a in element]
        node_list_1d.append(self.root.keys[0])
        node_list_1d = sorted(node_list_1d)
        spaced_list = " ".join([str(r) for r in node_list_1d])
        print(spaced_list)
